import tensorflow as tf, sys, cv2, numpy as np

# image_path = sys.argv[1]
# global image_path
image_path = '/home/abhyudai/Desktop/OCR/samples/test_C1.jpg'
# image_data = cv2.resize(cv2.imread(image_path, 0), (20, 20))
# image_data = np.reshape(image_data, (1, 20 * 20))
def recognise(image_path):
    # Read in the image_data
    image_data = tf.gfile.FastGFile(image_path, 'rb').read()
    # image_data = tf.image.decode_jpeg(image_path)
    ...
    with tf.Session() as sess:
        ...
        # print('%s (score = %.5f)' % (label_lines[top_k[0]], predictions[0][top_k[0]])), image_data
